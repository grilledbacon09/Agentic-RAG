...
class ConversationalAgent:
    ...
    def _apply_extraction(self, session: ConversationSession, message: str) -> List[str]:
        ...
        if session.pending_slot:
            self._mark_slots_confirmed(session, session.pending_slot)
    ...
    def _polish_message(self, draft: str, user_message: str) -> str:
    ...
    def _run_recommendation(
        ...
        *,
        ...
        if result.is_emergency:
            ...
            message = self._polish_message(draft, user_text or context)
    ...
    def _handle_follow_up(self, session: ConversationSession, intent: UserIntent) -> str:
        ...
        if intent == UserIntent.FOLLOW_UP_WHY:
            return format_follow_up_why(result)
    ...
    def process_message(
        ...
        if next_slot:
            ack = ""
            if session.pending_slot:
                ack = format_slot_ack(session, session.pending_slot) + "\n\n"
            message = ack + SLOT_QUESTIONS[next_slot]
