...
def _decode_taboo_segment(seg: bytes, *, first: bool, last: bool) -> dict | None:
...
def iter_taboo_items_from_part(path: Path) -> Iterator[dict]:
    """DUR taboo part.1 (대용량)을 mmap으로 항목 단위 스트리밍합니다."""
    path = Path(path)
    splitter = b'}, {"DUR_SEQ"'
    with path.open("rb") as f:
        mm = mmap.mmap(f.fileno(), 0, access=mmap.ACCESS_READ)
        try:
            anchor = mm.find(b'"items":')
            if anchor < 0:
                return
            arr_start = mm.find(b"[", anchor)
            if arr_start < 0:
                return
            pos = arr_start + 1
            end = mm.rfind(b"]")
            ...
            first = True
            while pos < end:
                idx = mm.find(splitter, pos, end)
                if idx < 0:
                    obj = _decode_taboo_segment(mm[pos:end], first=first, last=True)
                    if obj:
                        yield obj
                    break
                obj = _decode_taboo_segment(mm[pos : idx + 1], first=first, last=False)
                if obj:
                    yield obj
                pos = idx + 3
                first = False
